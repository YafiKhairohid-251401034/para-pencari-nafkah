...
void OrderManager::setQuantity(int itemId, int qty) {
    int idx = findLine(itemId);
    if (idx >= 0) {
        if (qty <= 0) m_lines.removeAt(idx);
        else          m_lines[idx].quantity = qty;
        emit cartChanged();
    }
}
...
int OrderManager::itemCount() const {
    int n = 0;
    for (const auto& l : m_lines) n += l.quantity;
    return n;
}

int OrderManager::subtotal() const {
    int s = 0;
    for (const auto& l : m_lines) s += l.subtotal();
    return s;
}
...
QString OrderManager::formattedSubtotal() const { return formatRp(subtotal()); }
QString OrderManager::formattedTax()      const { return formatRp(tax()); }
QString OrderManager::formattedTotal()    const { return formatRp(total()); }

int OrderManager::findLine(int itemId) const {
    for (int i = 0; i < m_lines.size(); ++i)
        if (m_lines[i].item.id == itemId) return i;
    return -1;
}
...
QVector<MenuItem> OrderManager::allMenuItems() {
    return {
        ...
        {2,  "Iced Americano",         "Espresso blend, served cold",        32000, MenuCategory::Coffee,    "🧊", "#8BB5C8"},
        {3,  "Kopi Susu Gula Aren",    "Gula aren, full cream",              45000, MenuCategory::Coffee,    "🍯", "#C8A050"},
        ...
        // --- NON-COFFEE ---
        {11, "Matcha Milk",            "Premium green tea, full cream",      38000, MenuCategory::NonCoffee, "🍵", "#7AAF70"},
        ...
        {16, "Mango Smoothie",         "Fresh tropical blend",               35000, MenuCategory::NonCoffee, "🥭", "#E8A830"},

        // --- CAKE ---
        {17, "Butter Croissant",       "Flaky, 100% French butter",          28000, MenuCategory::Cake,      "🥐", "#E8C870"},
        ...
        // --- FOOD ---
        {23, "Nasi Goreng Kimchi",     "Spicy fusion, egg, spice",           55000, MenuCategory::Food,      "🍳", "#E87840"},
        ...
        {28, "Mushroom Bruschetta",    "Sauteed, fresh herbs",               42000, MenuCategory::Food,      "🍄", "#A08060"},
    };
}
