...
public:
  explicit OrderLineWidget(const OrderLine &line, QWidget *parent = nullptr)
      : QFrame(parent), m_itemId(line.item.id) {
...
OrderPage::OrderPage(OrderManager *mgr, QWidget *parent)
    : QWidget(parent), m_mgr(mgr), m_currentCategory(MenuCategory::All) {
  setupUi();
  setupStyle();
...
void OrderPage::refreshPage() {
  ...
  if (m_customerBadge) {
    m_customerBadge->setText(QString("👤 %1  |  🪑 Meja %2")
                                 .arg(m_mgr->customerName())
                                 .arg(m_mgr->tableNumber()));
  }
  refreshOrderPanel();
}
...
void OrderPage::buildSidebar() {
  ...
  brandIcon->setObjectName("sidebarIcon");
  brandIcon->setAlignment(Qt::AlignCenter);
  brandIcon->setFixedHeight(36);
  ...
  connect(m_navRegister, &QPushButton::clicked, this,
          &OrderPage::navigateToRegister);
  connect(m_navPayment, &QPushButton::clicked, this,
  ...
  connect(m_searchEdit, &QLineEdit::textChanged, [this](const QString &text) {
    ...
    for (int i = 0; i < m_menuGrid->count(); ++i) {
      ...
      if (MenuCard *card = qobject_cast<MenuCard *>(it->widget())) {
        bool match =
            text.isEmpty() ||
            card->item().name.contains(text, Qt::CaseInsensitive) ||
...
void OrderPage::onItemAdded(const MenuItem &item) { m_mgr->addItem(item); }

void OrderPage::onCartChanged() { refreshOrderPanel(); }

void OrderPage::refreshOrderPanel() {
  // Update customer label
  m_orderCustomer->setText(QString("%1  ·  Meja %2")
                               .arg(m_mgr->customerName())
                               .arg(m_mgr->tableNumber()));
  ...
  if (lines.isEmpty()) {
    ...
    for (const OrderLine &line : lines) {
      ...
      connect(w, &OrderLineWidget::removeItem, m_mgr,
